...
if( ($desde == ""  && $hasta == "") || ($desde > 0  && $hasta == "") ) { ?>
	...
	<?php } else if($desde > $hasta) {?>

		<div class="alert alert-warning">
			<h4 align="center">
				<strong> [!ERROR EN LAS FECHAS INGRESADAS¡] Favor corregir </strong>
			</h4>
		</div>
	<?php } else if(mysqli_num_rows($result)==0) {?>
		<div class="alert alert-warning">
			<h4 align="center">
				...
				<?php } else {?>
					No Encontramos Registros desde <strong><?php echo $desdeNor?></strong> hasta <strong><?php echo $hastanor ?></strong>
			</h4>
				<?php } ?>
		</div>
	<?php } else { ?>
			...
			<div class="panel-body">
				...
				<table  class="table bordered">
					<thead>
						<tr>
							<th colspan="11">
								<h3  align="center" >DETALLE DE ATENCIONES DE LOS MÉDICOS </h3>
								<h4>Total de Atenciones: <span class="badge"><?php echo $total ?></span></h4>
							</th>
						</tr>
						<tr>
							<th >ID</th>
					...
					<?php while( $row = mysqli_fetch_array($result) ) { ?>
						<tr>
							...
							<td>
								<button type="button" class="btn btn-primary " data-toggle="modal" data-target="#Detalle" onclick="Detalle('<?php echo $row['sl_idInfo']?>')" >
									<span class="glyphicon glyphicon-paperclip"></span> Detalle
								</button>
							</td>
						</tr>
					<?php  }
